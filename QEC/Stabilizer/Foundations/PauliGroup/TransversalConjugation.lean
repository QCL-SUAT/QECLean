...
/-- The bundled transversal gate coerces to its matrix (v4.34 port note: the
coercion no longer reduces during `simp`, so name it as a rewrite rule). -/
lemma uniformTransversalGate_coe (n : ℕ) (G : OneQubitGate) :
    (uniformTransversalGate n G : Matrix (NQubitBasis n) (NQubitBasis n) ℂ) =
      uniformTransversalGateMatrix n G := rfl
...
lemma uniformTransversalGate_conj_op_gate_of_localRule
  ...
  simpa [conjByGate_val, pointwiseScalarProduct, uniformTransversalGate_coe]
    using hmat
...
def xSupport (op : NQubitPauliOperator n) : Finset (Fin n) :=
  -- v4.34: bind the `DecidablePred` instance explicitly; the lambda's `op i`
  -- is no longer seen through the `NQubitPauliOperator` abbreviation during
  -- instance search.
  letI : DecidablePred (fun i => op i = PauliOperator.X) := fun _ => inferInstance
...
lemma uniformTransversalGate_inv_S_conj_Z_op_gate (n : ℕ) (op : NQubitPauliOperator n)
  ...
  apply Subtype.ext
  simpa [conjByGate_val, uniformTransversalGate_coe] using
    uniformTransversalGateMatrix_inv_S_conj_Z_op n op h
...
lemma uniformTransversalGate_inv_S_conj_XI_op_gate
    ...
    (conjByGate (uniformTransversalGate n inv_S) op.toGate).val =
      ((invSConjXIScalar op) • (invSConjXIImage op).toMatrix) := by
  simpa [conjByGate_val, uniformTransversalGate_coe] using
    uniformTransversalGateMatrix_inv_S_conj_XI_op n op hXI
...
lemma uniformTransversalGate_inv_S_conj_element_XI_phase0_gate
    ...
    (conjByGate (uniformTransversalGate n inv_S) g.gate).val =
      (invSConjXIScalar g.operators) •
        (NQubitPauliGroupElement.ofOperator (invSConjXIImage g.operators)).toMatrix := by
  simpa [conjByGate_val, NQubitPauliGroupElement.gate_val, uniformTransversalGate_coe] using
