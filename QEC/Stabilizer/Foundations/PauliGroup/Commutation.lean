...
/-- `2 * m` copies of `2` vanish in `Fin 4` (v4.34 port: the surrounding
proofs used to get this from `simp [nsmulRec]` + `omega`, which no longer see
through the `nsmulRec` recursion). -/
private lemma nsmulRec_two_mul_two_eq_zero (m : ℕ) :
    (nsmulRec (2 * m) 2 : Fin 4) = 0 := by
  induction m with
  | zero => decide
  | succ m ih =>
    have h : 2 * (m + 1) = 2 * m + 1 + 1 := by ring
    rw [h, nsmulRec_succ (2 * m + 1) (by omega) (2 : Fin 4) (by decide)]
    rcases Nat.eq_zero_or_pos m with hm | hm
    · subst hm; decide
    · have h2 : 2 * m + 1 = 2 * m + 1 := rfl
      rw [nsmulRec_succ (2 * m) (by omega) (2 : Fin 4) (by decide), ih]
      decide

/-- An odd number of `2`'s is `2` in `Fin 4` (companion of
`nsmulRec_two_mul_two_eq_zero`). -/
private lemma nsmulRec_two_mul_add_one (m : ℕ) :
    (nsmulRec (2 * m + 1) 2 : Fin 4) = 2 := by
  rcases Nat.eq_zero_or_pos m with hm | hm
  · subst hm; decide
  · rw [nsmulRec_succ (2 * m) (by omega) (2 : Fin 4) (by decide),
      nsmulRec_two_mul_two_eq_zero m]
    decide
...
lemma commutes_iff_even_anticommutes (p q : NQubitPauliGroupElement n) :
  ...
  · intro h;
    ...
    ((q.operators i).mulOp (p.operators i)).phasePower) := by
      -- v4.34: `convert … using 1` cannot unfold `mulOp`; rewrite with the
      -- defining equation instead.
      rw [mulOp_phasePower, mulOp_phasePower] at h_phase_diff
      exact h_phase_diff
...
lemma anticommutes_iff_odd_anticommutes (p q : NQubitPauliGroupElement n) :
  ...
  · intro h_phase
    ...
    % 2 = 1 := by
      ...
      [ nsmulRec_two_mul_two_eq_zero ] ] at * ;
      simp_all
    convert h_odd using 4
    -- v4.34: the residual goal is the defining equation of `anticommutesAt`.
    · rfl
  · intro h_odd
    ...
    erw [ Fin.val_mk ];
    induction ( Finset.card _ / 2 ) with
    | zero => decide
    | succ n ih =>
      rw [show 1 + 2 * (n + 1) = 2 * (n + 1) + 1 by ring]
      rw [nsmulRec_two_mul_add_one]
      decide
