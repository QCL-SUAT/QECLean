...
def support (op : NQubitPauliOperator n) : Finset (Fin n) :=
  -- v4.34: supply the `DecidablePred` argument explicitly; instance search no
  -- longer sees through the `NQubitPauliOperator` abbreviation in the lambda.
  @Finset.filter (Fin n) (fun i => op i ≠ PauliOperator.I)
    (fun _ => inferInstance) Finset.univ
