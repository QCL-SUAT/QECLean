...
/-- `phasePowerToComplex` converts a finite sum of phases into a product
(v4.34 port: stated once here instead of relying on an inline induction). -/
lemma phasePowerToComplex_sum_finset {ι : Type*} (s : Finset ι) (f : ι → Fin 4) :
    (∏ i ∈ s, PauliGroupElement.phasePowerToComplex (f i)) =
      PauliGroupElement.phasePowerToComplex (∑ i ∈ s, f i) := by
  classical
...
lemma toMatrix_mul (p q : NQubitPauliGroupElement n) :
  (p * q).toMatrix = p.toMatrix * q.toMatrix := by
    ...
    have h_phasePowerToComplex :
      ...
      -- v4.34 port: `convert` leaves two residuals — the definitional
      -- phase-power identity (`rfl`), and the product-vs-sum form, which the
      -- general lemma closes.
      all_goals first
        | rfl
        | rw [phasePowerToComplex_sum_finset Finset.univ
...
lemma NQubitPauliOperator.trace_matrix_mul (p q : NQubitPauliOperator n) :
  (p.toMatrix * q.toMatrix).trace = ∏ i, ((p i).toMatrix * (q i).toMatrix).trace := by
    ...
    * (∏ i : Fin n, (q i).toMatrix (k i) (b i)) = ∏ i : Fin n, ∑ b : QubitBasis,
    ...
    -- v4.34: unfold the trace and the product side explicitly; `convert` can no
    -- longer see these definitional equalities.
