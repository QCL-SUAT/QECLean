...
theorem hRowAt_independent_on_dualCycles (c : Z¹ L) (y0 y1 : Fin L) :
  ...
  have h_row_eq : ∀ y : Fin L, hRowAt L y c.1 + hRowAt L (next L y) c.1 = 0 := by
    ...
    have h_sum_zero : ∑ x : Fin L,
      ...
      -- v4.34: unfold `toricDualBoundary` with its named equation instead of
      -- relying on `convert`'s implicit reduction.
      simpa [toricDualBoundary_apply] using congr_arg
        ( fun f : FaceIdx L → ZMod 2 => ∑ x : Fin L, f ( x, y ) )
        h_dual_boundary_zero
    have h_sum_zero :
  ...
  have hvColAt_linear : ∀ x : Fin L,
    ...
    have h_sum : ∑ y : Fin L,
      ...
      have h_sum : ∀ y : Fin L,
        ...
        have hmem : (toricDualBoundary L) c.val = 0 := by
          simpa only [toricDualCycles, LinearMap.mem_ker] using c.2
        simpa [toricDualBoundary_apply] using congr_fun hmem (x, y)
...
theorem vColAt_dualBoundary_zero (b : B¹ L) :
  ...
  · rw [ ← Equiv.sum_comp ( Equiv.addRight 1 ) ] ; simp +decide [ Fin.add_def, Nat.mod_eq_of_lt ];
    ...
    -- v4.34: the goal lives on `ZMod.val`; take values of the `ZMod 2` identity.
    have hv := h (∑ x : Fin (L + 1 + 1), s (zeroCoord (L + 1 + 1),
        ⟨(↑x + 1) % (L + 1 + 1), Nat.mod_lt _ (by omega)⟩))
    -- v4.34: the goal lives on the `Fin`-val representation; bridge via `Fin.val_add`.
    rw [← Fin.val_add, hv, Fin.val_zero]
...
theorem toric_finrank_dualCycles :
  ...
  have h_dual_boundary_range :
    ...
    have h_dual_boundary :
      ...
      -- v4.34: `convert Iff.rfl` leaves instance-equality side goals here;
      -- prove the two directions explicitly instead.
      -- v4.34: bridge the `toMatrix'`-wrapped range to the raw one by a
...
theorem phiDual_injective :
  ...
  have h_finrank :
    ...
    have hfun : Function.Surjective (phiDualLinearMap (L := L)) := by
      intro y
      obtain ⟨x, hx⟩ := this y
      refine ⟨x, ?_⟩
      rw [← phiDual_eq_phiDualLinearMap (L := L) x]
      exact hx
    rw [ LinearMap.range_eq_top.mpr hfun ] <;> norm_num [ this ];
  have := LinearMap.finrank_range_add_finrank_ker ( phiDualLinearMap ( L := L ) ) ;
  simp_all +decide ;
  convert LinearMap.ker_eq_bot.mp this using 1
  -- v4.34: the residual is `phiDual = ⇑phiDualLinearMap`, proved pointwise.
  funext x
  exact phiDual_eq_phiDualLinearMap (L := L) x
