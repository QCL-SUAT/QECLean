...
theorem toric_rank_nullity_boundary1 :
  ...
  -- v4.34: `simpa` leaves an `HAdd` instance mismatch on `ℕ` (see Code.lean);
  -- `omega` works up to the instances.
  have h := LinearMap.finrank_range_add_finrank_ker (∂₁ (L := L))
  rw [toricCycles]
  omega
...
theorem toric_rank_nullity_boundary2 :
    dim₂ (C2 L) =
      dim₂ (LinearMap.ker (∂₂ (L := L))) +
        dim₂ (B₁ L) := by
  -- v4.34: same `HAdd` instance mismatch as `toric_rank_nullity_boundary1`.
  have h := LinearMap.finrank_range_add_finrank_ker (∂₂ (L := L))
  rw [toricBoundaries]
  omega
...
/-- v4.34 port note: `δ⁰`'s `toFun` is a structure-literal `match`, which `simp`
no longer unfolds at implicit transparency. These two equation lemmas (both `rfl`)
restore the evaluation behaviour the proofs below rely on. -/
@[simp] lemma toricVertexCutMap_h (s : C0 L) (x y : Fin L) :
    (δ⁰ (L := L)) s (EdgeIdx.h x y) = s (x, y) + s (next L x, y) := rfl

@[simp] lemma toricVertexCutMap_v (s : C0 L) (x y : Fin L) :
    (δ⁰ (L := L)) s (EdgeIdx.v x y) = s (x, y) + s (x, next L y) := rfl
...
-- (400000 in the v4.30 original; raised for v4.34).
  ...
  · intro hf
    ...
    have h_const_x :
      ...
      have h_eq :
        ...
        -- v4.34: `convert congr_arg … hf using 1` no longer sees the defining
        -- equation of `∂₂` (structure-literal `match`); supply it explicitly.
        have hmem : (∂₂ (L := L)) f = 0 := by simpa only [LinearMap.mem_ker] using hf
        have h := congr_fun hmem (EdgeIdx.v x ⟨0, by linarith [Fact.out (p := 0 < L)]⟩)
        rw [show (∂₂ (L := L)) f (EdgeIdx.v x ⟨0, by linarith [Fact.out (p := 0 < L)]⟩) =
            f (x, ⟨0, by linarith [Fact.out (p := 0 < L)]⟩) +
              f (prev L x, ⟨0, by linarith [Fact.out (p := 0 < L)]⟩) from rfl] at h
        simpa using h
      have h_eq :
