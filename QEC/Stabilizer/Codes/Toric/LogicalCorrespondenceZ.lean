...
/-- Defining equation of `toricDualBoundary` at a face (v4.34 port: `simp` no
longer reduces the structure-literal application to the four-term sum). -/
lemma toricDualBoundary_apply (L : ℕ) [Fact (0 < L)] (c : C1 L) (x y : Fin L) :
    toricDualBoundary L c (x, y) =
      c (EdgeIdx.h x y) + c (EdgeIdx.h x (next L y)) +
        c (EdgeIdx.v x y) + c (EdgeIdx.v (next L x) y) := rfl
...
theorem toricHomologicalCode_dualBoundary_eq :
  ...
  have hh :
    ...
    have h1 :
      ...
      · simp
      · intros p _ hne; rw [ite_eq_right hne]; ring
