    ...
    have h_sum_zero :
      ...
      have h_diff : ∀ y : Fin L,
        ...
        have h_cycle : ∂₁ (L := L) c.val (x, y) = 0 := by
          have h : (∂₁ (L := L)) c.val = 0 := by
            simpa only [toricCycles, LinearMap.mem_ker] using c.2
          simpa using congr_fun h (x, y)
        -- v4.34: `convert … using 1` no longer unfolds `∂₁`; rewrite with its
        -- defining equation instead.
        rw [toricBoundary1_apply] at h_cycle
        exact h_cycle
      aesop;
  ...
  have h_B1_gen : ∀ y : Fin L, vAt (L := L) y (c.1) = vAt (L := L) (prev (L := L) y) (c.1) := by
    ...
    have h_sum_eq :
      ...
      have h_sum_eq : ∀ x : Fin L,
        ...
        have h : (∂₁ (L := L)) c.val = 0 := by
          simpa only [toricCycles, LinearMap.mem_ker] using c.2
        have hc : (∂₁ (L := L)) c.val (x, y) = 0 := by
          simpa using congr_fun h (x, y)
        rw [toricBoundary1_apply] at hc
        exact hc
      aesop;
...
theorem phi_eq_phiLinearMap (x : H₁ L) :
  ...
  -- v4.34: `convert rfl` can no longer see the definitional unfolding here.
  rfl
