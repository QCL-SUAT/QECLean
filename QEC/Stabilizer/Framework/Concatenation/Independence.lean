...
lemma rowsLinearIndependent_promotedOuterList
  ...
  have hcoeff : ∀ b : Fin n₂,
    ...
    have hsum2 : (∑ j, (![c1, c2]) j
      ...
      rw [Fin.sum_univ_two]
      -- v4.34: state the reduced form explicitly; the `![…]`/`checkMatrix`
      -- reductions no longer fire under `simpa`.
      change c1 • (D.Cin.logicalX 0).operators.toSymplectic +
          c2 • (D.Cin.logicalZ 0).operators.toSymplectic = 0
      exact h1
