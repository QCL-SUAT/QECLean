...
theorem rank_nullity_boundary1 :
  ...
  -- v4.34: `simpa [cycles, add_comm]` no longer closes this — the two `+` on `ℕ`
  -- carry different (defeq but not syntactically equal) `HAdd` instances, and
  -- simplification does not normalize them. `omega` works up to the instances.
  have h := LinearMap.finrank_range_add_finrank_ker X.boundary1
  rw [cycles]
  omega
...
theorem rank_nullity_boundary2 :
    dim₂ (X.C2 → ZMod 2) =
      dim₂ (LinearMap.ker X.boundary2) +
        dim₂ X.boundaries := by
  -- v4.34: see `rank_nullity_boundary1` — same `HAdd` instance mismatch.
  have h := LinearMap.finrank_range_add_finrank_ker X.boundary2
  rw [boundaries]
  omega
