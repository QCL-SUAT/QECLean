...
lemma convEquiv_one_add_single {G : Type} [Fintype G] [AddCommGroup G]
  ...
  congr 1
  · rw [AddMonoidAlgebra.one_def, AddMonoidAlgebra.coeff_single_apply, Pi.single_apply]
    exact if_congr eq_comm rfl rfl
  · rw [AddMonoidAlgebra.coeff_single_apply, Pi.single_apply]
    exact if_congr eq_comm rfl rfl
...
theorem elementForm_of_orderFourLift {Ghat : Type}
  ...
  have hρsurj : Function.Surjective
      ⇑(AddMonoidAlgebra.mapDomainRingHom (ZMod 2) q) := by
    -- v4.34: `AddMonoidAlgebra` is a Finsupp wrapper, so surjectivity is
    -- transported through `coeff`/`ofCoeff` rather than `Finsupp.mapDomain`.
    intro y
    obtain ⟨x, hx⟩ := Finsupp.mapDomain_surjective hsurj (AddMonoidAlgebra.coeff y)
    refine ⟨AddMonoidAlgebra.ofCoeff x, ?_⟩
    rw [← AddMonoidAlgebra.ofCoeff_coeff y]
    exact congrArg AddMonoidAlgebra.ofCoeff hx
