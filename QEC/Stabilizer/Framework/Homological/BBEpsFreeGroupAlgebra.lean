...
lemma span_transversal_eq_top
  ...
  classical
  intro s _
  -- v4.34: `AddMonoidAlgebra` is a structure around `M →₀ R`; the coefficient
  -- Finsupp is `s.coeff`, and `sum_coeff_single` replaces `Finsupp.sum_single`.
  have hs : s = s.coeff.sum (fun g c => AddMonoidAlgebra.single g c) :=
    (AddMonoidAlgebra.sum_coeff_single s).symm
  rw [hs]
  ...
  have key : (AdjoinRoot.mk ((X : k[X]) ^ N)
        (Polynomial.C (s.coeff g) * (X + 1) ^ (j : ℕ))) •
        AddMonoidAlgebra.single (t i) (1 : k)
      = AddMonoidAlgebra.single g (s.coeff g) := by
...
lemma linearIndependent_transversal
  ...
  have hcoeff : ∀ m < N,
    ...
    have happ : (∑ i ∈ l.support, ∑ j ∈ Finset.range N,
        ...
        = (0 : G →₀ k) (t i₀ + m • σ) := by
      -- v4.34: `hl'` lives in the group algebra, so go through `.coeff` before
      -- evaluating at the group element.
      have h := congrArg (fun v : AddMonoidAlgebra k G => v.coeff (t i₀ + m • σ)) hl'
      simpa using h
