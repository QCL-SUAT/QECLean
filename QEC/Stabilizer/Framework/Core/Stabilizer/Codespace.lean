...
lemma mul_stabilizerSum_eq (S : StabilizerGroup n) (g : NQubitPauliGroupElement n)
  ...
  · -- v4.34: `convert` leaves both sides; `simp only [stabilizerSum]` no longer
    -- unfolds the definition, so each side is handled explicitly.
    simp only [stabilizerSum, Finset.mul_sum _ _ _]
    exact Finset.sum_congr rfl fun x hx => by
      exact Eq.symm (NQubitPauliGroupElement.toMatrix_mul g x)
  · rw [stabilizerSum]
