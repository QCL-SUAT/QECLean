...
-- v4.34: `Equivalence` is a structure, not a class, so declaring this as an
-- `instance` is now a (fatal) style error; it is a plain lemma.
lemma sameLogicalOperator_equivalence (S : StabilizerGroup n) :
    Equivalence (fun L L' => SameLogicalOperator L L' S) where
  refl L := refl L S
  symm := symm S
  trans := trans S
