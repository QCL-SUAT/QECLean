    ...
    · have := this g hg
      ...
      · -- v4.34: the coercion of `U • ⟨…⟩` no longer unfolds under `rw`, so
        ...
        change (g.toMatrix * (U : Matrix (NQubitBasis n) (NQubitBasis n) ℂ)) *ᵥ v =
          √(∑ i, ‖v i‖ ^ 2) •
            (g.toMatrix *ᵥ ((U : Matrix (NQubitBasis n) (NQubitBasis n) ℂ) *ᵥ
              ((√(∑ i, ‖v i‖ ^ 2))⁻¹ • v)))
        rw [Matrix.mulVec_mulVec, Matrix.mulVec_smul, smul_smul,
      ...
      · change (U : Matrix (NQubitBasis n) (NQubitBasis n) ℂ) *ᵥ v =
          √(∑ i, ‖v i‖ ^ 2) •
            ((U : Matrix (NQubitBasis n) (NQubitBasis n) ℂ) *ᵥ
              ((√(∑ i, ‖v i‖ ^ 2))⁻¹ • v))
